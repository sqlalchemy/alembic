...
class PostgresqlOpTest(TestBase):
    ...
    def test_add_identity_to_column(
        ...
        if pass_existing_server_default:
            alter_column_kw["existing_server_default"] = None
        # else: leave existing_server_default at its default, which is the
        # ``False`` "not specified" sentinel, not ``None``. It must be
        # treated the same as an explicit ``None``, or the identity is
        # never actually added.
        # Regression test for
        # https://github.com/sqlalchemy/alembic/issues/1504
