...
class PostgresqlOpTest(TestBase):
    ...
    def test_add_identity_to_column_no_existing_server_default(self):
        """existing_server_default left at its default (``False``, meaning
        "not specified") must be treated the same as an explicit ``None``,
        or the identity is never actually added.

        Regression test for https://github.com/sqlalchemy/alembic/issues/1504
        """
        context = op_fixture("postgresql")
        op.alter_column(
            "t1",
            "some_column",
            server_default=Identity(on_null=True),
        )
        context.assert_(
            "ALTER TABLE t1 ALTER COLUMN some_column ADD "
