...
def type_bound_check_constraint_names(table: Table) -> set[str]:
    """Returns the names of check constraints on the given table that
    are type-bound (e.g. generated by ``Boolean``/``Enum`` with
    ``create_constraint=True``), including those on columns.

    These are the constraints excluded by ``all_table_check_constraints()``
    on the metadata side; on the reflection side they still come back from
    the database, so callers need their names to exclude the matching
    reflected constraints as well.
    """
    candidates = chain(
        table.constraints, *(c.constraints for c in table.columns)
    )
    return {
        ...
        and _is_type_bound(ck)
        and ck.name is not None
    }
