...
* ``type`` - this is the type of hook we are running.  Alembic includes
  three hook runners: ``"console_scripts"``, which is specifically a
  Python function that uses ``subprocess.run()`` to invoke a separate
  Python script against the revision file; ``"exec"``, which uses
  ``subprocess.run()`` to execute an arbitrary binary; and ``"module"``,
  which uses ``subprocess.run()`` to invoke a Python module directly.
